...
def post_Player():
    ...
    score = 0
    hi_score = get_Players(name)
    new_player = Player(name, datetim, gameid, score)
...
def get_Players(qname):
    ...
    for players in all_players:
        ...
        if players.name == qname:
            if players.score > new_high: new_high = players.score
    gameid = new_gameid + 1
...
@app.route('/putScore', methods=['PUT'])
def put_score():
    global gameid
    score = request.json["score"]
    newid = db.session.query(Player.id).filter(Player.gameid == gameid).first()
    player = Player.query.get(newid)
    player.score = score
    db.session.commit()
    return str(gameid)
